...
const LiquidMetalMaterial = shaderMaterial(
    ...
    void main() {
      vUv = uv;
      vPosition = position;
      // Fluid Distortion Pattern - FASTER FLOW
      // Increased speed to ensure the effect is noticeable
      float n = snoise(vec3(position.x * 1.1, position.y * 1.1, uTime * 0.25));
      
      // Final Displacement - DEEPER
      // Deeper contrast for shadows
      float displacement = n * 0.8;
  ...
  `,
    ...
    void main() {
      // 1. Base Metal/Liquid Mix
      // Sharper contrast to define the "chrome" edges clearly
      float mixFactor = smoothstep(-0.2, 0.8, vDisplacement * 2.0 + 0.1);
      vec3 color = mix(uColor2, uColor1, mixFactor); 
      
      // 2. Shadows/AO
      // Deeper valleys for 3D pop
      color -= vec3(max(0.0, -vDisplacement) * 0.45); 
      
      // 3. Specular Highlight
      // Brighter, sharper shine to catch the eye
      ...
      float aberration = vDisplacement * 0.03;
      vec3 r = color * (1.0 + aberration);
      vec3 g = color;
      vec3 b = color * (1.0 - aberration);
      
      // 5. Fresnel Effect
      // Strong rim light to highlight the text area
      float fresnel = pow(1.0 - dot(vNormal, vec3(0.0, 0.0, 1.0)), 2.0);
      
      vec3 finalColor = vec3(r.r, g.g, b.b);
      
      // Add sheen
      finalColor += vec3(1.0) * fresnel * 0.7;
        ...
        >
            ...
            >
                <Suspense fallback={null}>
                    {/* Environment removed to prevent blue tint */}
